...
- (void)applicationDidFinishLaunching: (NSNotification *)aNotification {
  ...
  pidFileName = [NSString stringWithFormat: @"%@%@.app.pid", NSTemporaryDirectory(), processInfo.processName];
  NSString *pid = [NSString stringWithFormat: @"%i", processInfo.processIdentifier];
  [pid writeToFile: pidFileName atomically: YES encoding: NSUTF8StringEncoding error: nil];
