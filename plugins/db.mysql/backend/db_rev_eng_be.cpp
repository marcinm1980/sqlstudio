...
auto Db_rev_eng::grtm() -> void {
  Db_plugin::grtm(true);
  Sql_import::grtm();
}

auto Db_rev_eng::task_desc() -> std::string {
  return "Reverse engineer database";
}

auto Db_rev_eng::target_catalog() -> db_CatalogRef {
  return Db_plugin::model_catalog();
}

auto Db_rev_eng::sql_script() -> std::string {
  std::string sql_script;
  Db_plugin::dump_ddl(sql_script);
  return sql_script;
}

auto Db_rev_eng::parse_sql_script(parsers::MySQLParserServices::Ref sql_parser, parsers::MySQLParserContext::Ref context,
...
auto Db_rev_eng::getVersion() -> GrtVersionRef {
  std::string value;
