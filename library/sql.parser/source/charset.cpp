...
static auto add_collation(CHARSET_INFO *cs) -> int {
                   ...
                   (cs->number=get_collation_number_internal(cs->name))))
  {
    ...
    {
      CHARSET_INFO *new_= all_charsets[cs->number];
      ...
      if (!strcmp(cs->csname,"ucs2") )
      {
        ...
        new_->cset= my_charset_ucs2_general_uca.cset;
        new_->coll= my_charset_ucs2_general_uca.coll;
        new_->strxfrm_multiply= my_charset_ucs2_general_uca.strxfrm_multiply;
        new_->min_sort_char= my_charset_ucs2_general_uca.min_sort_char;
        new_->max_sort_char= my_charset_ucs2_general_uca.max_sort_char;
        new_->mbminlen= 2;
        new_->mbmaxlen= 2;
        new_->state |= MY_CS_AVAILABLE | MY_CS_LOADED;
#endif        
      }
      else
      {
        ...
        */
        if (sort_order && sort_order[(uchar)'A'] < sort_order[(uchar)'a'] &&
                          sort_order[(uchar)'a'] < sort_order[(uchar)'B'])
...
//  char fname[FN_REFLEN];
  ...
  */
  if (!charset_initialized)
  {
    ...
    */
//xxx    pthread_mutex_lock(&THR_LOCK_charset);
    if (!charset_initialized)
    {
      ...
      // absent charsets dir in linux causes crash. compiled charsets should be enough.
      ////strmov(get_charsets_dir(fname), MY_CHARSET_INDEX);
      //strcpy(get_charsets_dir(fname), MY_CHARSET_INDEX);
      //error= my_read_charset_file(fname,myflags);
      charset_initialized=1;
    }
//xxx    pthread_mutex_unlock(&THR_LOCK_charset);
  }
  return error;
}
