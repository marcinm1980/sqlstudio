...
struct TimerTask {
  int task_id;
  double next_time;      // Precomputed target time when this task must be triggered again.
  double wait_time;      // The time in seconds to wait until this task is executed again.
...
private:
  std::mutex _timer_lock; // Synchronize access to the timer class.
  std::unique_ptr<ThreadPool> _pool;      // A number of threads which trigger the callbacks (to make them independant of each other).
  ...
  std::thread _thread; // This thread loops endlessly executing tasks as they come in.
  TaskList _tasks;

  ThreadedTimer(int base_frequency);
  ~ThreadedTimer();

  static void start(ThreadedTimer& thread);
  static void pool_function(ThreadedTimer& thread);
  void main_loop();
