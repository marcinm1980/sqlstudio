...
public:
  ...
  static auto create(bec::GridModel::Ref model, GridView *view, const std::string &name) -> Ref;
  ~GridViewModel();

  virtual auto handle_popup_event(GdkEvent *event) -> bool;
  auto refresh(bool reset_columns) -> int;
  auto column_index(Gtk::TreeViewColumn *col) -> int;
  auto row_numbers_visible(bool value) -> void {
    _row_numbers_visible = value;
  }
  auto row_numbers_visible() -> bool {
  ...
  auto ignore_column_resizes(bool flag) -> void {
    if (flag)
  ...
  auto onColumnsResized(const std::vector<Gtk::TreeViewColumn *> &cols) -> void;
...
private:
  ...
  auto get_cell_value(const iterator &iter, int column, GType type, Glib::ValueBase &value) -> void;
  auto set_cell_value(const iterator &itier, int column, GType type, const Glib::ValueBase &value) -> void;

  auto on_column_header_button_press(GdkEventButton *ev, Gtk::TreeViewColumn *col) -> void;
};
