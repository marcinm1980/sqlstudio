...
namespace MySQL.GUI.MySqlStudio
{
  public partial class ModelOverviewForm : TabDocument, IMySqlStudioDocument
  {
    ...
    private MySqlStudioMenuManager studioMenuManager;
    ...
    public ModelOverviewForm(WbContext WbContext, Overview be)
    {
      ...
      UpdateTabText();
      studioMenuManager = new MySqlStudioMenuManager(wbContext);
      ...
      if (panel != null)
      {
        if (panel != null && panel.Tag != null)
        {
          ...
          if (items != null && items.Count > 0)
          {
            ...
            menu = studioMenuManager.ShowContextMenu(panel, items, e.X, e.Y, new EventHandler(PopupActionHandler));
            menu.Tag = panel;
      ...
      if (listView != null && e.Button == MouseButtons.Right)
      {
        ...
        if (item_nodes.Count != 0)
        {
          ...
          if (items.Count > 0)
          {
            ...
            menu = studioMenuManager.ShowContextMenu(listView, items, e.X, e.Y, new EventHandler(PopupActionHandler));
            menu.Tag = listView;
