...
namespace MySQL.GUI.MySqlStudio
{
  public partial class MainForm : Form, IMySqlStudioObserver
  {
    ...
    // Special Forms
    private ModelOverviewForm studioPhysicalOverviewForm = null;
    ...
    {
      get { return studioPhysicalOverviewForm; }
    }
    ...
    public void RefreshGUI(RefreshType refresh, String str, IntPtr ptr)
    {
      ...
      switch (refresh)
      {
        case RefreshType.RefreshCloseDocument:
          {
            ...
            if (studioPhysicalOverviewForm != null)
            {
              studioPhysicalOverviewForm.ResetDocument(true);
              studioPhysicalOverviewForm.Close();
              studioPhysicalOverviewForm = null;
            }
        ...
        case RefreshType.RefreshOverviewNodeChildren:
          if (studioPhysicalOverviewForm != null)
            studioPhysicalOverviewForm.RefreshGUI(refresh, str, ptr);
          break;
      }
    ...
    public void DestroyView(BaseWindowsCanvasView canvasView)
    {
      ...
      if (modelDiagramForm != null)
      {
        ...
        if (studioPhysicalOverviewForm != null)
          studioPhysicalOverviewForm.Activate();
      }
    }
    ...
    private bool EditCanUndo()
    {
      ...
      if (wbDocument == studioPhysicalOverviewForm)
        ...
        if (wbDocument != null)
        {
          ...
          if (searchString != "")
          {
            if (!wbContext.try_searching_diagram(searchString))
            {
              ...
              if (contentTabControl.ActiveDocument == studioPhysicalOverviewForm)
                studioPhysicalOverviewForm.SearchAndFocusNode(searchString);
            }
          }
        }
      }
    ...
    {
      if (studioPhysicalOverviewForm == null)
        studioPhysicalOverviewForm = new ModelOverviewForm(wbContext, wbContext.get_physical_overview());

      //SuspendLayout();
      //contentTabControl.SuspendLayout();
      DockDocument(studioPhysicalOverviewForm, true, true);
      Update();
      if (isNew)
        studioPhysicalOverviewForm.RebuildModelContents();
    ...
    private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
    {
      ...
      // Go through each studio document and close it, now unconditionally.
      contentTabControl.SuspendLayout();
      try
      {
        ITabDocument[] documents = contentTabControl.DocumentsToArray();
