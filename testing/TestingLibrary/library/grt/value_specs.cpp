...
void test_list_value(grt::ListRef<ItemType>& lv, grt::Ref<ItemType> v[]) {
  ...
  $expect([&]() { lv.insert(v[6], 7); }).toThrowError<grt::bad_item>("Index out of range");
  ...
  $expect([&]() { lv.set(6, v[6]); }).toThrowError<grt::bad_item>("Index out of range");
  $expect(lv.count()).toEqual(6U);
...
$describe("GRT values") {
  ...
  $it("operator == ()", [&]() {
    ...
    EXPECT_EQ(i1234 == i1234, true);
    EXPECT_FALSE(i1234 != i1234);
    EXPECT_EQ(i1234, grt::IntegerRef(1234));
    $expect(i1234 != grt::IntegerRef(1234)).toBeFalse();
    EXPECT_EQ(i1234, 1234);
    EXPECT_FALSE(i1234 != 1234);
    $expect(i1234).Not.toEqual(0);
    EXPECT_FALSE(i1234 == 0);
    EXPECT_FALSE(i1234 == i1235);
    ...
    EXPECT_EQ(d1234, d1234);
    EXPECT_FALSE(d1234 != d1234);
    EXPECT_EQ(d1234, grt::DoubleRef(123.4));
    $expect(d1234 != grt::DoubleRef(123.4)).toBeFalse();
    EXPECT_EQ(d1234, 123.4);
    EXPECT_FALSE(d1234 != 123.4);
    $expect(d1234).Not.toEqual(0.0);
    EXPECT_FALSE(d1234 == 0.0);
    EXPECT_FALSE(d1234 == d1235);
    ...
    EXPECT_EQ(sbar, sbar);
    EXPECT_FALSE(sbar != sbar);
    EXPECT_EQ(sbar, grt::StringRef("bar"));
    $expect(sbar != grt::StringRef("bar")).toBeFalse();
    EXPECT_EQ(sbar, "bar");
    $expect(sbar).Not.toEqual("baz");
    EXPECT_FALSE(sbar == sbaz);
    ...
    o3 = o1;
    //    EXPECT_EQ(o1, o3);          TODO: Fix this...it's throwing an exception
    EXPECT_FALSE(o1 != o3);

    tmp1 = o1;
    tmp2 = o1;
    //    EXPECT_EQ(tmp1, tmp2);      TODO: Fix this...it's throwing an exception
    EXPECT_FALSE(tmp1 != tmp2);
    tmp2 = o2;
    EXPECT_FALSE(tmp1 == tmp2);
    ...
    for (int i = 0; i < 6; i++) {
      for (int j = 0; j < 6; j++) {
        if (i < j) {
          ...
          EXPECT_FALSE(x[j] < x[i]);
        } else if (i > j) {
          $expect(x[j]).toBeLessThan(x[i]);
          EXPECT_FALSE(x[i] < x[j]);
        } else {
          EXPECT_FALSE(x[j] < x[i] || x[i] < x[j]);
        }
      }
    }
  });
