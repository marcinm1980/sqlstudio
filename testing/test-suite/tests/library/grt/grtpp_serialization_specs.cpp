...
protected:
  ...
  void SetUp() override {
    ...
    EXPECT_EQ(GRT::get()->get_metaclasses().size(), 6U);
...
TEST_F(GRTSerializationTest, SerializationOfSimpleValuesAndDictionaries) {
  ...
  runSerialization(iv);
  runSerialization(dv);
  runSerialization(ValueRef(sv));
  runSerialization(ValueRef(iv));
  runSerialization(ValueRef(dv));
...
TEST_F(GRTSerializationTest, SerializationOfListsWithNULLValues) {
  ...
  grt::GRT::get()->serialize(list, outputDir + "/null_list.xml");

  list = grt::ListRef<db_Table>::cast_from(grt::GRT::get()->unserialize(outputDir + "/null_list.xml"));
