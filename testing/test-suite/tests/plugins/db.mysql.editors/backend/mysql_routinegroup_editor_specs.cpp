...
#define NL "\n"
   ...
   */
TEST_F(MySQLRoutineGroupEditorTest, EditorWithInvalidRoutines) {
  const char* routine_sql =
  ...
  EXPECT_EQ(5U, double_processed_routines.size()) << "Lines unintentionally removed";

  // Now compares each routine to discard any difference
  for (size_t index = 0; index < processed_routines.size(); index++) {
    EXPECT_EQ(double_processed_routines[index], processed_routines[index]) << "Routine unintentionally changed";
  }
}
}
