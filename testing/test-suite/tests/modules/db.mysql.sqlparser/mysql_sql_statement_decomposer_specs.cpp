...
namespace {
  ...
  struct TestData {
    ...
    void test_sql(const std::string &sql, const std::string &master_result) {
      ...
      EXPECT_EQ(master_result, oss.str());
    }
    ...
    void test_view(const std::string &sql, const std::string &master_result) {
      ...
      EXPECT_EQ(1, res) << base::strfmt("failed to parse statement: %s", sql.c_str());

      std::ostringstream oss;
      oss << *select_statement;
      EXPECT_EQ(master_result, oss.str()) << sql;
    }
  };

class SqlStatementDecomposerTest : public ::testing::Test {
protected:
...
TEST_F(SqlStatementDecomposerTest, Query1) {
