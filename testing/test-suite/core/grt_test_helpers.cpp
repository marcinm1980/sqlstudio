...
void compareValues(std::string const& major_msg, grt::ValueRef actualValue, grt::ValueRef expectedValue,
  ...
  EXPECT_EQ(grt_type_to_str(actualValue.type()), grt_type_to_str(expectedValue.type())) << msg + ", different types";

  switch (actualValue.type()) {
    case grt::IntegerType: {
      EXPECT_EQ(*grt::IntegerRef::cast_from(actualValue), *grt::IntegerRef::cast_from(expectedValue)) << msg + ", different values";
      break;
    }

    case grt::DoubleType: {
      EXPECT_EQ(*grt::DoubleRef::cast_from(actualValue), *grt::DoubleRef::cast_from(expectedValue)) << msg + ", different values";
      break;
    }

    case grt::StringType: {
      EXPECT_EQ(*grt::StringRef::cast_from(actualValue), *grt::StringRef::cast_from(expectedValue)) << msg + ", different values";
      break;
    }
    ...
    case grt::ObjectType: {
      ...
      EXPECT_EQ(actualMetaData->name(), expectedMetaData->name()) << msg + ", meta class names differ";
    ...
    case grt::ListType: {
      ...
      EXPECT_EQ(actualList.count(), expectedList.count()) << msg + ", list sizes differ";
    ...
    case grt::DictType: {
      ...
      EXPECT_EQ(actualDict.count(), expectedDict.count()) << msg + ", dict sizes differ";
      ...
      for (auto actualIterator = actualDict.begin(), expectedIterator = expectedDict.begin();
        ...
        EXPECT_EQ(actualIterator->first, expectedIterator->first) << msg + ", dict keys differ";
