...
class DiffSQLGeneratorBE {
  ...
  auto generate_create_stmt(db_mysql_TriggerRef, bool for_alter = false) -> void;
  void generate_create_stmt(db_mysql_ViewRef);
  auto generate_create_stmt(db_mysql_RoutineRef, bool for_alter = false) -> void;
  void generate_create_stmt(db_mysql_SchemaRef);
  void generate_create_stmt(db_mysql_CatalogRef);
  auto generate_create_stmt(db_UserRef user) -> void;
  ...
  auto generate_drop_stmt(db_mysql_TriggerRef, bool for_alter = false) -> void;
  void generate_drop_stmt(db_mysql_ViewRef);
  auto generate_drop_stmt(db_mysql_RoutineRef, bool for_alter = false) -> void;
  void generate_drop_stmt(db_mysql_SchemaRef);
  void generate_drop_stmt(db_mysql_CatalogRef);
  auto generate_drop_stmt(db_UserRef user) -> void;

  // table columns
  auto generate_alter(grt::ListRef<db_mysql_Column> columns, const grt::MultiChange *) -> void;

  // table indices
  auto generate_alter(grt::ListRef<db_mysql_Index> indices, const grt::MultiChange *diffchange) -> void;

  /**
   * Create/remove table FKs
   */
  auto generate_add_fk(db_mysql_ForeignKeyRef fk) -> std::string;
  auto generate_drop_fk(db_mysql_ForeignKeyRef fk) -> std::string;
  auto generate_alter(grt::ListRef<db_mysql_ForeignKey> fks, const grt::MultiChange *diffchange) -> void;
  auto generate_alter_drop(grt::ListRef<db_mysql_ForeignKey> fks, const grt::MultiChange *diffchange) -> void;
   ...
   */
  auto generate_create_partitioning(db_mysql_TableRef table) -> void;
   ...
   */
  auto generate_set_partitioning(db_mysql_TableRef table, const grt::DiffChange *diffchange) -> void;
   ...
   * list or map.
   */
  auto generate_alter_stmt(db_mysql_CatalogRef, const grt::DiffChange *) -> void;
  auto generate_alter_stmt(db_mysql_SchemaRef, const grt::DiffChange *) -> void;
  ...
  auto generate_alter_stmt(db_mysql_TableRef, const grt::DiffChange *, AlterTableFlags alter_table_flags) -> void;
  // some drops needs to go prior to the rest of alters, e.g. drop FKs to allow column rename in other tables
  auto generate_alter_stmt_drops(db_mysql_TableRef, const grt::DiffChange *) -> void;
  auto generate_alter_stmt(db_mysql_ViewRef old_view, db_mysql_ViewRef new_view, const grt::DiffChange *diffchange) -> void;
  auto generate_routine_alter_stmt(db_mysql_RoutineRef old_routine, db_mysql_RoutineRef new_routine,
                                   const grt::DiffChange *diffchange) -> void;

  auto process_trigger_alter_stmts(db_mysql_TableRef table, const grt::DiffChange *triggers_cs) -> void;

  auto do_process_diff_change(grt::ValueRef org_object, grt::DiffChange *) -> void;
...
public:
   ...
   */
  auto process_diff_change(grt::ValueRef org_object, grt::DiffChange *, grt::DictRef) -> void;
  /**
   * function that starts generating SQL into a list
   */
  auto process_diff_change(grt::ValueRef org_object, grt::DiffChange *, grt::StringListRef,
                           grt::ListRef<GrtNamedObject>) -> void;
};
