...
bool ModelFile::attempt_xml_document_upgrade(xmlDocPtr xmldoc, const std::string &version) {
  ...
  if (major == 1 && minor == 1) {
    ...
    if (revision == 0) {
      ...
      std::vector<xmlNodePtr> views(xml.scan_objects_of_type("studio.physical.View"));
      ...
      for (size_t c = views.size(), i = 0; i < c; i++) {
        ...
        set_grt_object_item_link(page_settings, "owner", studio_physical_Diagram::static_class_name(), id.c_str());
...
#undef RENAME_FIELDS
    ...
    // version 1.1.6 -> 1.2.0
    // * studio.model.Layer -> studio.physical.Layer
    if (revision == 6) {
      const char *attribs[] = {"struct-name", "content-struct-name", NULL};
      const char *from[] = {"studio.model.Layer", NULL};
      const char *to[] = {"studio.physical.Layer", NULL};
  ...
  if (major == 1 && minor == 2) {
    ...
    // studio.physical.View -> studio.physical.Diagram
    // studio.logical.View -> studio.logical.Diagram

    // member names (key)
    // model.Model:currentView -> currentDiagram
    // model.Model:views -> diagrams
    // model.Marker:view -> diagram
    // studio.physical.Model:views -> diagrams
    // studio.logical.Model:views -> diagrams
    ...
    {
      const char *attribs[] = {"struct-name", "content-struct-name", NULL};
      const char *from[] = {"model.View", "studio.physical.View", "studio.logical.View", NULL};
      const char *to[] = {"model.Diagram", "studio.physical.Diagram", "studio.logical.Diagram", NULL};

      find_replace_xml_attributes(xmlDocGetRootElement(xmldoc), attribs, from, to);
    }

    {
      const char *klass[] = {"model.Model",  "studio.physical.Model", "studio.logical.Model", "model.Model",
                             "model.Marker", "studio.physical.Model", "studio.logical.Model", NULL};
    ...
    {
      const char *klass[] = {"studio.physical.Layer",
                             "studio.physical.TableFigure",
                             "studio.physical.ViewFigure",
                             "studio.physical.RoutineGroupFigure",
                             "studio.model.NoteFigure",
                             "studio.model.ImageFigure",
                             NULL};
...
studio_DocumentRef ModelFile::attempt_document_upgrade(const studio_DocumentRef &doc, xmlDocPtr xmldoc,
  ...
  if (major <= 1 && minor <= 1 && revision <= 2) {
    if (doc->physicalModels().count() > 0) {
      ListRef<studio_physical_Diagram> views(doc->physicalModels()[0]->diagrams());
...
static void check_figure_layers(studio_physical_DiagramRef view, std::list<std::string> &load_warnings) {
  ...
  for (ssize_t i = view->figures().count() - 1; i >= 0; --i) {
    ...
    if (figure.is_instance<studio_physical_TableFigure>()) {
      if (!studio_physical_TableFigureRef::cast_from(figure)->table().is_valid()) {
        load_warnings.push_back(
          strfmt("Table figure %s is not attached to any database table and was deleted.", figure->name().c_str()));
        orphan = true;
      }
    } else if (figure.is_instance<studio_physical_ViewFigure>()) {
      if (!studio_physical_ViewFigureRef::cast_from(figure)->view().is_valid()) {
        load_warnings.push_back(
          strfmt("View figure %s is not attached to any database table and was deleted.", figure->name().c_str()));
        orphan = true;
      }
    } else if (figure.is_instance<studio_physical_RoutineGroupFigure>()) {
      if (!studio_physical_RoutineGroupFigureRef::cast_from(figure)->routineGroup().is_valid()) {
...
void ModelFile::check_and_fix_inconsistencies(const studio_DocumentRef &doc, const std::string &version) {
  ...
  for (size_t c = models.count(), i = 0; i < c; i++) {
    studio_physical_ModelRef model(models[i]);
    for (size_t vc = model->diagrams().count(), v = 0; v < vc; v++) {
      studio_physical_DiagramRef diagram(model->diagrams()[v]);
