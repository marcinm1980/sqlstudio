...
WBContext::WBContext(bool verbose) : _frontendCallbacks(nullptr) {
  ...
  _studio = grt::GRT::get()->get_native_module<MySqlStudioImpl>();
  _studio->set_context(this);
...
WBContext::~WBContext() {
  ...
  //{
  //  studio_MySqlStudioRef app(get_root());
...
auto WBContext::software_rendering_enforced() -> bool {
  ...
  if (!_force_opengl_rendering) {
    ...
    std::string videoAdapter = grt::StringRef::cast_from(_studio->call_function("getVideoAdapter", arguments));
  ...
  if (get_root()->options()->options().get_int("studio:ForceSWRendering", 0) != 0)
    result = true;
...
auto WBContext::init_(WBFrontendCallbacks *callbacks, WBOptions *options) -> bool {
  ...
  logInfo("System info:\n %s\n", _studio->getSystemInfo(true).c_str());
...
auto WBContext::init_finish_(WBOptions *options) -> void {
  ...
  if (initial_file.empty() && get_wb_options().get_int("studio.AutoReopenLastModel", 0)) {
...
auto WBContext::set_default_options(grt::DictRef options) -> void {
  ...
  set_default(options, "studio:AutoSaveModelInterval", AUTO_SAVE_MODEL_INTERVAL);
  set_default(options, "studio:AutoSaveSQLEditorInterval", AUTO_SAVE_SQLEDITOR_INTERVAL);
  set_default(options, "studio.AutoReopenLastModel", 0);
  set_default(options, "studio:SaveSQLWorkspaceOnClose", 1);
  set_default(options, "studio:InternalSchema", ".mysqlstudio");

  set_default(options, "studio.physical:DeleteObjectConfirmation", "ask");
  ...
  // Model Defaults
  set_default(options, "DefaultFigureNotation", "studio/default");
  ...
  // Default Colors
  set_default(options, "studio.model.Layer:Color", "#F0F1FE");
  set_default(options, "studio.model.NoteFigure:Color", "#FEFDED");

  set_default(options, "studio.physical.Diagram:DrawLineCrossings", 0);
  set_default(options, "studio.physical.ObjectFigure:Expanded", 1);
  set_default(options, "studio.physical.TableFigure:ShowColumnTypes", 1);
  set_default(options, "studio.physical.TableFigure:ShowColumnFlags", 0);
  set_default(options, "studio.physical.TableFigure:MaxColumnTypeLength", 20);
  set_default(options, "studio.physical.TableFigure:MaxColumnsDisplayed", 30);
  set_default(options, "studio.physical.RoutineGroupFigure:MaxRoutineNameLength", 20);

  set_default(options, "studio.physical.TableFigure:Color", "#98BFDA");
  set_default(options, "studio.physical.ViewFigure:Color", "#FEDE58");
  set_default(options, "studio.physical.RoutineGroupFigure:Color", "#98D8A5");

  // Default Fonts

  set_default(options, "studio.physical.FontSet:Name", "Default (Western)");

  set_default(options, "studio.physical.TableFigure:TitleFont", DEFAULT_FONT_FAMILY " Bold 12");
  set_default(options, "studio.physical.TableFigure:SectionFont", DEFAULT_FONT_FAMILY " Bold 11");
  set_default(options, "studio.physical.TableFigure:ItemsFont", DEFAULT_FONT_FAMILY " 11");
  set_default(options, "studio.physical.ViewFigure:TitleFont", DEFAULT_FONT_FAMILY " Bold 12");
  set_default(options, "studio.physical.RoutineGroupFigure:TitleFont", DEFAULT_FONT_FAMILY " Bold 12");
  set_default(options, "studio.physical.RoutineGroupFigure:ItemsFont", DEFAULT_FONT_FAMILY " 12");
  set_default(options, "studio.physical.Connection:CaptionFont", DEFAULT_FONT_FAMILY " 11");
  set_default(options, "studio.physical.Layer:TitleFont", DEFAULT_FONT_FAMILY " 11");
  set_default(options, "studio.model.NoteFigure:TextFont", DEFAULT_FONT_FAMILY " 11");

#if defined(_MSC_VER)
  set_default(options, "studio.general.Resultset:Font", DEFAULT_FONT_FAMILY " 8");
  if (get_local_os_name().find("Windows XP") != std::string::npos) {
    set_default(options, "studio.general.Editor:Font", DEFAULT_MONOSPACE_FONT_FAMILY_ALT " 10");
    set_default(options, "studio.scripting.ScriptingShell:Font", DEFAULT_MONOSPACE_FONT_FAMILY_ALT " 10");
    set_default(options, "studio.scripting.ScriptingEditor:Font", DEFAULT_MONOSPACE_FONT_FAMILY_ALT " 10");
  } else {
    set_default(options, "studio.general.Editor:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 10");
    set_default(options, "studio.scripting.ScriptingShell:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 10");
    set_default(options, "studio.scripting.ScriptingEditor:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 10");
  }
#elif defined(__APPLE__)
  set_default(options, "studio.general.Resultset:Font", DEFAULT_FONT_FAMILY " 11");
  set_default(options, "studio.general.Editor:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 13");
  set_default(options, "studio.scripting.ScriptingShell:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 13");
  set_default(options, "studio.scripting.ScriptingEditor:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 13");
#else
  set_default(options, "studio.general.Resultset:Font", DEFAULT_FONT_FAMILY " 11");
  set_default(options, "studio.general.Editor:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 11");
  set_default(options, "studio.scripting.ScriptingShell:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 11");
  set_default(options, "studio.scripting.ScriptingEditor:Font", DEFAULT_MONOSPACE_FONT_FAMILY " 11");
#endif
  ...
  colors += "#F0F1FE\n";
  colors += "#FFEBFA\n";
  set_default(options, "studio.model.Figure:ColorList", colors);
  ...
  // Other options
  set_default(options, "studio.physical.Connection:ShowCaptions", 0);
  set_default(options, "studio.physical.Connection:CenterCaptions", 0);
...
auto WBContext::setLogLevelFromGuiPreferences(const grt::DictRef &dict) -> void {
  ...
  std::string prefsLogLevel = dict.get_string("studio.logger:LogLevel", currentLogLevel);
...
auto WBContext::close_document_finish() -> void {
  ...
  get_root()->doc(studio_DocumentRef());
...
auto WBContext::cleanup_options() -> void {
  ...
  grt::DictRef options = get_root()->options()->options();
  options.remove("studio.physical.ConnectionFigure:CaptionFont");
  options.remove("studio.model.Layer:TitleFont");
  options.remove("studio.model.NoteFigure:TitleFont");
  options.remove("studio.physical:DeleteObjectConfirmation");
...
auto WBContext::execute_plugin_grt(const app_PluginRef &plugin, const grt::BaseListRef &args) -> grt::ValueRef {
  ...
  if (plugin.is_instance(app_DocumentPlugin::static_class_name())) {
    ...
    studio_DocumentRef document(get_document());
    bool flag= false;
