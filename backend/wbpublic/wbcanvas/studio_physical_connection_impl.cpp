...
#include "studio_physical_connection_impl.h"
...
studio_physical_Connection::ImplData::ImplData(studio_physical_Connection *owner) : super(owner) {
  _highlighting = false;
...
auto studio_physical_Connection::ImplData::set_in_view(bool flag) -> void {
  ...
  if (flag && !_line && diagram->owner().is_valid() &&
      studio_physical_ModelRef::cast_from(diagram->owner())->get_data()->get_relationship_notation() ==
...
auto studio_physical_Connection::ImplData::set_foreign_key(const db_ForeignKeyRef &fk) -> void {
  ...
  if (owner_valid && self()->_foreignKey.is_valid())
    studio_physical_DiagramRef::cast_from(self()->owner())
      ->get_data()
  ...
  if (owner_valid && fk.is_valid())
    studio_physical_DiagramRef::cast_from(self()->owner())->get_data()->add_fk_mapping(fk, self());
...
auto studio_physical_Connection::ImplData::update_connected_tables() -> void {
  ...
  if (self()->owner().is_valid()) {
    ...
    if (table.is_valid() && reftable.is_valid()) {
      studio_physical_DiagramRef view(studio_physical_DiagramRef::cast_from(self()->owner()));
...
auto studio_physical_Connection::ImplData::get_start_canvas_item() -> mdc::CanvasItem * {
  if (self()->_foreignKey.is_valid()) {
    ...
    if (studio_physical_ModelRef::cast_from(self()->owner()->owner())->get_data()->get_relationship_notation() ==
          PRFromColumnNotation &&
...
auto studio_physical_Connection::ImplData::realize() -> bool {
...

  {
    ...
    if (studio_physical_TableFigureRef::cast_from(self()->_startFigure)->table() == self()->_foreignKey->owner()) {
      _table_changed_conn =
        db_TableRef::cast_from(self()->_foreignKey->owner())
          ->signal_refreshDisplay()
          ->connect(std::bind(&studio_physical_Connection::ImplData::table_changed, this, std::placeholders::_1));
    } else {
      _table_changed_conn = self()->_foreignKey->referencedTable()->signal_refreshDisplay()->connect(
        std::bind(&studio_physical_Connection::ImplData::table_changed, this, std::placeholders::_1));
    }
    ...
    scoped_connect(_line->signal_layout_changed(),
                   std::bind(&studio_physical_Connection::ImplData::layout_changed, this));
