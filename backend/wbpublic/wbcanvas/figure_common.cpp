...
auto Titlebar::on_button_press(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
...
auto Titlebar::on_button_release(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                 mdc::EventState state) -> bool {
...
auto Titlebar::on_enter(mdc::CanvasItem *target, const Point &point) -> bool {
  if (!_hub || !_hub->figure_enter(_owner->represented_object(), target, point))
    return super::on_enter(target, point);
  return false;
}

auto Titlebar::on_leave(mdc::CanvasItem *target, const Point &point) -> bool {
...
auto Titlebar::on_click(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button, mdc::EventState state) -> bool {
...
auto Titlebar::on_double_click(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                               mdc::EventState state) -> bool {
...
auto Titlebar::set_icon(cairo_surface_t *icon) -> void {
  _icon_text.set_icon(icon);
}

auto Titlebar::set_title(const std::string &text) -> void {
  _icon_text.set_text(text);
}

auto Titlebar::set_expanded(bool flag) -> void {
  if (_expander && _expander->get_active() != flag)
    _expander->set_active(flag);
}

auto Titlebar::get_expanded() -> bool {
  return _expander->get_active();
}

auto Titlebar::expand_toggled() -> void {
  _signal_expand_toggle(_expander->get_active());
}

auto Titlebar::set_color(const Color &color) -> void {
  _back_color = color;
  set_needs_render();
}

auto Titlebar::set_text_color(const Color &color) -> void {
  _icon_text.set_pen_color(color);
}

auto Titlebar::set_border_color(const Color &color) -> void {
  _border_color = color;
}

auto Titlebar::set_font(const mdc::FontSpec &font) -> void {
  _icon_text.set_font(font);
}

auto Titlebar::set_rounded(mdc::CornerMask corners) -> void {
  _corners = corners;
}

auto Titlebar::render(mdc::CairoCtx *cr) -> void {
  cr->save();
...
auto Titlebar::set_auto_sizing(bool flag) -> void {
...
auto FigureItem::on_button_press(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                 mdc::EventState state) -> bool {
...
auto FigureItem::on_button_release(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                   mdc::EventState state) -> bool {
...
auto FigureItem::on_enter(mdc::CanvasItem *target, const Point &point) -> bool {
  if (target != this || !_hub->figure_enter(_owner->represented_object(), target, point))
    return super::on_enter(target, point);
  return false;
}

auto FigureItem::on_leave(mdc::CanvasItem *target, const Point &point) -> bool {
...
auto FigureItem::on_click(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button, mdc::EventState state) -> bool {
...
auto FigureItem::on_double_click(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                 mdc::EventState state) -> bool {
...
auto FigureItem::get_intersection_with_line_to(const Point &p) -> Point {
...
auto BaseFigure::on_button_press(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                 mdc::EventState state) -> bool {
...
auto BaseFigure::on_button_release(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                   mdc::EventState state) -> bool {
...
auto BaseFigure::on_enter(mdc::CanvasItem *target, const Point &point) -> bool {
  if (!_hub->figure_enter(represented_object(), target, point))
    return super::on_enter(target, point);
  return false;
}

auto BaseFigure::on_leave(mdc::CanvasItem *target, const Point &point) -> bool {
  if (!_hub->figure_leave(represented_object(), target, point))
    return super::on_leave(target, point);
  return false;
}

auto BaseFigure::on_click(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button, mdc::EventState state) -> bool {
  if (target != this || !_hub->figure_click(represented_object(), target, point, button, state))
    return super::on_click(target, point, button, state);
  return false;
}

auto BaseFigure::on_double_click(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                 mdc::EventState state) -> bool {
...
auto BaseFigure::unset_color() -> void {
  set_draw_background(false);
}

auto BaseFigure::set_color(const Color &color) -> void {
  set_background_color(color);
  set_draw_background(true);
}

auto BaseFigure::highlight(const Color *color) -> void {
  set_highlight_color(color);
  set_highlighted(true);
}

auto BaseFigure::unhighlight() -> void {
  set_highlight_color(0);
  set_highlighted(false);
}

auto BaseFigure::set_allow_manual_resizing(bool flag) -> void {
...
auto BaseFigure::invalidate_min_sizes(mdc::CanvasItem *item) -> void {
...
auto BaseFigure::invalidate_min_sizes() -> void {
  invalidate_min_sizes(this);
}

auto BaseFigure::on_drag_handle(mdc::ItemHandle *handle, const Point &pos, bool dragging) -> bool {
  if (!_manual_resizing)
...
auto BaseFigure::set_content_font(const mdc::FontSpec &font) -> void {
  _content_font = font;
  if (font.family == "Arial")
    _content_font = font;
}

auto BaseFigure::set_state_drawing(bool flag) -> void {
  _disable_state_drawing = !flag;
}

static auto find_item(BaseFigure::ItemList &list, const std::string &id) -> BaseFigure::ItemList::iterator {
...
auto BaseFigure::begin_sync(mdc::Box &box, ItemList &list) -> BaseFigure::ItemList::iterator {
  get_layer()->get_view()->lock();
  get_layer()->get_view()->lock_redraw();
  return list.begin();
}

auto BaseFigure::sync_next(mdc::Box &box, ItemList &list, ItemList::iterator iter,
                                                     const std::string &id, cairo_surface_t *icon,
                                                     const std::string &text, const CreateItemSlot &create_item,
                                                     const UpdateItemSlot &update_item) -> BaseFigure::ItemList::iterator {
...
auto BaseFigure::end_sync(mdc::Box &box, ItemList &list, ItemList::iterator iter) -> void {
...
auto CaptionFigure::on_button_press(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                    mdc::EventState state) -> bool {
...
auto CaptionFigure::on_button_release(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                      mdc::EventState state) -> bool {
...
auto CaptionFigure::on_enter(mdc::CanvasItem *target, const Point &point) -> bool {
  if (!_hub->figure_enter(_owner_object, target, point))
    return super::on_enter(target, point);
  return false;
}

auto CaptionFigure::on_leave(mdc::CanvasItem *target, const Point &point) -> bool {
...
auto CaptionFigure::on_click(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                             mdc::EventState state) -> bool {
...
auto CaptionFigure::on_double_click(mdc::CanvasItem *target, const Point &point, mdc::MouseButton button,
                                    mdc::EventState state) -> bool {
...
auto ShrinkableBox::calc_min_size() -> Size {
  if (_manual_resizing) {
    if (_children.empty())
...
auto ShrinkableBox::resize_to(const Size &size) -> void {
  if (!_children.empty() &&
...
auto ShrinkableBox::set_item_count_limit(int limit) -> void {
  _limit_item_count = limit;

  resize_to(calc_min_size());
}

auto ShrinkableBox::set_allow_manual_resizing(bool flag) -> void {
...
auto ShrinkableBox::render(mdc::CairoCtx *cr) -> void {
  super::render(cr);
  if (_hidden_item_count > 0) {
    char text[100];
