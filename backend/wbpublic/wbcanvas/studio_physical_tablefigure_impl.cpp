...
#include "studio_physical_tablefigure_impl.h"
#include "studio_physical_diagram_impl.h"
#include "studio_physical_model_impl.h"
...
studio_physical_TableFigure::ImplData::ImplData(studio_physical_TableFigure *self) : super(self), _figure(0) {
...
void studio_physical_TableFigure::ImplData::update_options(const std::string &key) {
  ...
  if (base::hasPrefix(key, "studio.physical.ObjectFigure:") ||
      base::hasPrefix(key, "studio.physical.TableFigure:")) {
    if (_figure)
      sync_columns();

    if (key == "studio.physical.TableFigure:ShowSchemaName") {
      if (self()->owner()->owner()->get_data()->get_int_option("studio.physical.TableFigure:ShowSchemaName", 0) !=
          0) {
...
void studio_physical_TableFigure::ImplData::set_in_view(bool flag) {
  ...
  if (flag) {
    if (self()->_table.is_valid()) {
      studio_physical_DiagramRef diagram(studio_physical_DiagramRef::cast_from(self()->_owner));
...
void studio_physical_TableFigure::ImplData::set_table(const db_TableRef &table) {
  ...
  if (self()->_table.is_valid()) {
    if (self()->_owner.is_valid())
      studio_physical_DiagramRef::cast_from(self()->_owner)->get_data()->remove_mapping(self()->_table);
  ...
  if (self()->_table.is_valid()) {
    if (self()->_owner.is_valid())
      studio_physical_DiagramRef::cast_from(self()->_owner)->get_data()->add_mapping(table, self());
    ...
    if (_figure) {
      ...
      run_later(std::bind(&studio_physical_TableFigure::ImplData::sync_indexes, this));
      run_later(std::bind(&studio_physical_TableFigure::ImplData::sync_triggers, this));
    } else {
...
void studio_physical_TableFigure::ImplData::sync_columns() {
  if (_figure) {
    ...
    bool show_type =
      self()->owner()->owner()->get_data()->get_int_option("studio.physical.TableFigure:ShowColumnTypes", 1) != 0;
    bool show_flags =
      self()->owner()->owner()->get_data()->get_int_option("studio.physical.TableFigure:ShowColumnFlags", 0) != 0;
    int max_type_length =
      self()->owner()->owner()->get_data()->get_int_option("studio.physical.TableFigure:MaxColumnTypeLength", 20);
...
bool studio_physical_TableFigure::ImplData::realize() {
  ...
  if (!_figure) {
    ...
    update_options("studio.physical.TableFigure:MaxColumnsDisplayed");
    ...
    if (self()->owner()->owner()->get_data()->get_int_option("studio.physical.TableFigure:ShowSchemaName", 0) != 0) {
      std::string title = *self()->_table->owner()->name();
      title += ".";
      title += *self()->_table->name();
      _figure->get_title()->set_title(title);
    ...
    scoped_connect(_figure->get_title()->signal_expand_toggle(),
                   std::bind(&studio_physical_TableFigure::ImplData::toggle_title, this, std::placeholders::_1,
                             _figure->get_title()));
