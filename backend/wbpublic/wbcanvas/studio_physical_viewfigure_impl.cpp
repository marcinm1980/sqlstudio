...
#include "studio_physical_viewfigure_impl.h"
...
studio_physical_ViewFigure::ImplData::ImplData(studio_physical_ViewFigure *owner) : super(owner), _figure(0) {
  _resizable = false;
}
...
void studio_physical_ViewFigure::ImplData::set_in_view(bool flag) {
  ...
  if (flag) {
    if (self()->_view.is_valid())
      studio_physical_DiagramRef::cast_from(self()->_owner)->get_data()->add_mapping(self()->_view, self());
  } else {
    if (self()->_view.is_valid())
      studio_physical_DiagramRef::cast_from(self()->_owner)->get_data()->remove_mapping(self()->_view);
  }
...
void studio_physical_ViewFigure::ImplData::set_view(const db_ViewRef &view) {
  ...
  if (self()->view().is_valid()) {
    if (self()->_owner.is_valid())
      studio_physical_DiagramRef::cast_from(self()->_owner)->get_data()->add_mapping(view, self());
