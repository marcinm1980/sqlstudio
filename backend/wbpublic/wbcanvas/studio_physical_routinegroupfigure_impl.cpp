...
#include "studio_physical_routinegroupfigure_impl.h"
...
studio_physical_RoutineGroupFigure::ImplData::ImplData(studio_physical_RoutineGroupFigure *self)
...
void studio_physical_RoutineGroupFigure::ImplData::set_in_view(bool flag) {
  ...
  if (flag) {
    if (self()->_routineGroup.is_valid())
      studio_physical_DiagramRef::cast_from(self()->_owner)->get_data()->add_mapping(self()->_routineGroup, self());
  } else {
    if (self()->_routineGroup.is_valid())
      studio_physical_DiagramRef::cast_from(self()->_owner)->get_data()->remove_mapping(self()->_routineGroup);
  }
...
void studio_physical_RoutineGroupFigure::ImplData::set_routine_group(const db_RoutineGroupRef &rgroup) {
  ...
  if (self()->routineGroup().is_valid()) {
    rgroup->signal_contentChanged()->connect(
      std::bind(&studio_physical_RoutineGroupFigure::ImplData::contents_changed, this));

    if (self()->_owner.is_valid())
      studio_physical_DiagramRef::cast_from(self()->_owner)->get_data()->add_mapping(rgroup, self());
...
void studio_physical_RoutineGroupFigure::ImplData::sync_routines() {
  if (_figure) {
    ...
    int maxnlength = self()->owner()->owner()->get_data()->get_int_option(
      "studio.physical.RoutineGroupFigure:MaxRoutineNameLength", 20);
